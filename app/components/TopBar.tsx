...
export default function TopBar() {
  ...
  // 📢 Announcements ဆွဲယူမည့် Function
  const fetchAnnouncements = async () => {
    try {
      ...
      // စာအသစ်ရှိမရှိ စစ်မယ် (ဥပမာ- နောက်ဆုံးစာရဲ့ ID ကို localStorage မှာ မှတ်ထားလို့ရတယ်)
  ...
  useEffect(() => {
    ...
    // 🔥 Real-time Notification: Admin က DB မှာ စာအသစ်တင်တာနဲ့ ချက်ချင်းသိမယ်
  ...
  const handleNotiClick = () => {
    ...
    if (announcements.length > 0) {
      localStorage.setItem("last_noti_id", announcements[0].id);
    }
  };

  return (
    <div className="fixed top-0 left-0 w-full h-16 bg-[#0f0f0f]/90 backdrop-blur-md z-[99999] flex items-center justify-between px-4 border-b border-white/5 shadow-2xl overflow-visible">
  {/* 🔥 အရေးကြီးတာက z-[99999] နဲ့ overflow-visible ပါ ကိုကို */}
      {/* 1. Profile Avatar (ကိုကို့ကုဒ်အတိုင်း...) */}
     ...
     {/* 3. Right Side: Search & Notification */}
<div className="flex items-center gap-2">
  ...
  {/* 🔔 ခေါင်းလောင်းကို Link အဖြစ်ပြောင်းလိုက်ပြီ */}
  <Link href="/notifications" className="relative p-2 text-gray-300 hover:text-yellow-500 transition active:scale-95">
    ...
    {hasNew && (
      <span className="absolute top-2.5 right-2.5 w-2 h-2 bg-red-600 rounded-full ring-2 ring-[#0f0f0f] animate-pulse"></span>
    )}
  </Link>
</div>
